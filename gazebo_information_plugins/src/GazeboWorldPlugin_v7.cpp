    ...
    public:
      ...
      //  ros::ServiceServer service1,service2,service3,service4;
        ...
        std::vector<int> get_walls(std::string robot_name1, std::string robot_name2) {
            ...
            model1 = _world->ModelByName(robot_name1);
            model2 = _world->ModelByName(robot_name2);
            ...
            float x_1=model1->WorldPose().Pos().X();
            float y_1=model1->WorldPose().Pos().Y();
            float x_2=model2->WorldPose().Pos().X();
            float y_2=model2->WorldPose().Pos().Y();
          ...
          //  ROS_WARN("iterations %d \n", iterations);
            ...
            check = 1;
            _world->ModelByName(test_object)->SetStatic(true);
            ...
            for (int i = 0; i < iterations; i++) {
              _world->ModelByName(test_object)->SetStatic(true);
                ...
                ignition::math::Pose3d new_test_pose(start_x + i * increment*my_temp, start_y + i * increment * slope, 0.12, 0, 0, 0);
                _world->ModelByName(test_object)->SetWorldPose(new_test_pose);
              //  this->object_name = "";
                _world->ModelByName(test_object)->SetWorldPose(new_test_pose);
            ...
            vec.clear();
            ignition::math::Pose3d new_test_pose(200.0, 200.0, 10.0, 0, 0, 0);
            _world->ModelByName(test_object)->SetWorldPose(new_test_pose);
            return type_vec;
        ...
        float get_distance(std::string robot_name1, std::string robot_name2) {
            ...
            if (model1 == 0 || model2 == 0) {
                ...
                float x_diff = model1->WorldPose().Pos().X() - model2->WorldPose().Pos().X();
                float y_diff = model1->WorldPose().Pos().Y() - model2->WorldPose().Pos().Y();
